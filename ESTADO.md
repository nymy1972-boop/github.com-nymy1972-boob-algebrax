...
## Próximo paso
Si el usuario lo pide, recorrer las 6 pantallas del onboarding paso a paso. Después: confirmar con el usuario el resultado de la prueba real del webhook en Hotmart ("Testar webhook"); conectar `nymystudio.com` a Vercel si se quiere un link corto para Hotmart; y los pendientes de sesiones anteriores (dominio propio en Resend, migrar el resto de `lib/progress.ts` a Supabase, tabla `ai_calls`, activar "Leaked Password Protection" en Supabase Auth — 1 clic).
