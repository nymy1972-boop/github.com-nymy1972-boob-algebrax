...
VEREDICTO_ONBOARDING_NO_LISTA: la pantalla de diagnóstico del onboarding (`app/onboarding/page.tsx` + `components/onboarding/Diagnostico.tsx`) tiene veredicto **NO LISTA** en `docs/revisiones/onboarding-veredicto.md` (última medición: 29/40 usabilidad, 14/20 craft — el gate exige ≥36/40 y ≥16/20). Se corrigieron 5 defectos reales en 3 rondas de revisor-visual (28→29/40, 12→14/20): bug de fuente Baloo2/Jakarta no conectada a next/font, sombra "block-press" con poco contraste, hit-area del botón volver <44px, sin forma de saltar una pregunta trabada, feedback sin `aria-live`. Defectos restantes documentados en el veredicto: fondo plano en la zona de pregunta (falta un 3er nivel de profundidad), cero atajos de teclado (1-4 para elegir opción), y la sombra "block-press" no quedó unificada al 100% entre las 3 sub-pantallas (nombre/grado/dolor vs. diagnóstico). Se detuvo la iteración aquí (no se relanzó una 4ª ronda de revisor) porque el usuario pidió pasar a otras tareas explícitamente (isotipo, login) y el retorno por ronda venía cayendo. Próxima sesión que toque el onboarding: aplicar los 3 defectos restantes y re-lanzar `revisor-visual` sobre `docs/revisiones/onboarding-375.png` hasta pasar el gate doble.
