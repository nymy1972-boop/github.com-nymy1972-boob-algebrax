...
- ⚠️ **Pendiente del usuario (dashboard de Supabase, no código):** por defecto, Supabase manda un correo con SOLO un enlace para hacer clic — la pantalla `/entrar` pide escribir un código de 6 dígitos, así que hay que editar la plantilla de correo **"Magic Link"** en Authentication → Email Templates y agregar `{{ .Token }}` en el cuerpo del mensaje. Sin este paso, el correo llega pero no muestra el código a escribir (aunque el enlace del correo sí funcionaría si lo tocan). El usuario está ahí ahora, guiándolo paso a paso para encontrar la sección.
...
## Próximo paso
Terminar de guiar al usuario para agregar `{{ .Token }}` en la plantilla "Magic Link" de Supabase, luego probar el login completo de punta a punta con un correo real (pedir código → recibir correo → escribir código → entrar a `/app` con sesión real).
