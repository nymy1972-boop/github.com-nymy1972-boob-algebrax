...
## Próximo paso
Verificar con el usuario que el código de 8 dígitos (ahora sí aceptado por la pantalla) complete el login correctamente. Después, temas pendientes de sesiones futuras: verificar un dominio propio en Resend (para que cualquier estudiante reciba correo, no solo el dueño), conectar Hotmart, migrar `lib/progress.ts` de localStorage a las tablas de Supabase, agregar la tabla `ai_calls` para medir costo real de IA, y resolver el correo de soporte muerto + el botón de Google sin función.
