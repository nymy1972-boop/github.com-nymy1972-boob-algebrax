...
## Próximo paso
Conectar Resend como proveedor SMTP de Supabase (Authentication → Email Templates o Auth Settings → SMTP Settings, usando `onboarding@resend.dev` de arranque) — sin esto, el login por correo no envía nada, ni siquiera para pruebas. Después: agregar `{{ .Token }}` a la plantilla "Magic Link" (ahora si va a estar disponible) y probar el login de punta a punta con un correo real.
