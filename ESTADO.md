...
⚠️ **Pendiente crítico para que el pago active la cuenta solo**: falta configurar `HOTMART_HOTTOK` (la clave de seguridad del webhook) — sin eso, alguien puede pagar en Hotmart pero su cuenta no se sube a Premium sola. Se lo mencioné al usuario, esperando que confirme si quiere hacerlo ahora.

## Próximo paso
Conectar `HOTMART_HOTTOK` para que el webhook de Hotmart active Premium automáticamente tras el pago (bloqueante para vender de verdad). Después, temas pendientes de sesiones anteriores: verificar un dominio propio en Resend (para que cualquier estudiante reciba correo, no solo el dueño), migrar `lib/progress.ts` de localStorage a las tablas de Supabase, agregar la tabla `ai_calls` para medir costo real de IA, y resolver el correo de soporte muerto + el botón de Google sin función.
